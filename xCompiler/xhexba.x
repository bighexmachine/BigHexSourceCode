...
{ 
  ...
  prints("size: "); printn(codesize + mul((maxaddr - arraybase), 4)); newline()
...
{ sa := stringsize; 
  ...
  stringsize := stringsize + div(sl + 4, 4); 
  gen(cbf_string, 0, sa)
}
...
{ bufferp := 0; 
  ...
  { cb_unpack(bufferp);
    ...
    { cb_conststart := div(cb_loadpoint, 4);
      ...
      cb_loadpoint := cb_loadpoint + mul2(constp, 4) + mul2(tablesize, 4) + mul2(stringsize, 4)
    }
    else
    if flag = cbf_entry 
    then
...
{ loadstart := mul2(m_sp, 4);
  ...
  { cb_unpack(bufferp);
    ...
    { offset := cbv_low + cb_conststart;
      if cbv_high = r_areg
      then
        outcref(i_ldam, cbv_low)
      else
        outcref(i_ldbm, cbv_low);
      cb_loadpoint := cb_loadpoint + instlength(offset)
    }
    else
    if flag = cbf_table
...
proc outlab(t, lab) is
{ prlab(t, lab);
  newline()
}

proc outref(inst, lab) is
{ prints("  ");
  if inst = i_br
  then
    prints("BR      ")
...
{ count := 0;
  ...
  { outlab('S', count);
    len := div(rem(strings[count], 256), 4) + 1;
    while len > 0 do
    { outvar(strings[count]);
      count := count + 1;
      len := len - 1
    }
  }
